...
class Manuscript(db.Model):
    ...
    manuscript_id = db.Column(
        "manuscript_id", db.Integer, primary_key=True, autoincrement=True
    )
    editor_id = db.Column("editor_id", db.ForeignKey(Editor.editor_id), nullable=True)
...
class SalesRecord(db.Model):
    ...
    sales_record_id = db.Column(
        "sales_record_id", db.Integer, primary_key=True, autoincrement=True
    )
...
class Salesperson(db.Model):
    ...
    salesperson_id = db.Column(
        "salesperson_id", db.Integer, primary_key=True, autoincrement=True
    )
    first_name = db.Column("first_name", db.String(64), nullable=False)
    last_name = db.Column("last_name", db.String(64), nullable=False)
    salary = db.Column("salary", db.Integer, nullable=False)
...
# Defining these late because the 2nd Model subclass didn't exist until now.
Client.salesperson_id = db.Column(
    "salesperson_id", db.ForeignKey(Salesperson.salesperson_id), nullable=True
)
...
class AuthorMetadata(db.Model):
    ...
    author_metadata_id = db.Column(
        "author_metadata_id", db.Integer, primary_key=True, autoincrement=True
    )
...
Manuscript.series_id = db.Column(
    "series_id", db.ForeignKey(Series.series_id), nullable=True
)
...
AuthorsBooks = db.Table(
    ...
    db.Column(
        "book_id", db.ForeignKey(Book.book_id), primary_key=True, autoincrement=False
    ),
)
...
AuthorsManuscripts = db.Table(
    ...
    db.Column(
        "manuscript_id",
        db.ForeignKey(Manuscript.manuscript_id),
        primary_key=True,
        autoincrement=False,
    ),
)
