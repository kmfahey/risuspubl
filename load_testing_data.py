...
table_to_model_class = {
    "authors_metadata": AuthorMetadata,
    "authors": Author,
    "books": Book,
    "clients": Client,
    "editors": Editor,
...
def main():
    ...
    model_ids["authors"].extend(
        commit_model_objs_get_ids("authors", model_objs["authors"])
    )

    # Same again, with data/editors.tsv and the Editor class.
    model_objs["editors"].extend(read_file_get_model_objs("editors"))
    model_ids["editors"].extend(
        commit_model_objs_get_ids("editors", model_objs["editors"])
    )

    # Same again, with data/series.tsv and the Series class.
    model_objs["series"].extend(read_file_get_model_objs("series"))
    model_ids["series"].extend(
        commit_model_objs_get_ids("series", model_objs["series"])
    )
    ...
    for series_obj in model_objs["series"]:
        ...
        for i in range(0, series_obj.volumes):
            ...
            # pick again.
            while (
                book_obj.book_id in used_book_objs_ids or book_obj.series_id is not None
            ):
                book_obj = random.choice(model_objs["books"])
    ...
    model_ids["salespeople"].extend(
        commit_model_objs_get_ids("salespeople", model_objs["salespeople"])
    )
    ...
    for book_obj in model_objs["books"]:
        ...
        end_date = (
            date.today() if book_obj.is_in_print else generate_random_date(start_date)
        )
        sales_records_objs = generate_sales_record_objs(
            book_obj.book_id, start_date, end_date
        )
    ...
    for author_id in model_ids["authors"]:
        ...
        authors_metadata_argd[
            "photo_url"
        ] = "https://risuspublishing.com/cms/img/%s.jpeg" % (
...
def read_file_get_model_objs(table_name):
    ...
    for row in tsv_reader:
        ...
        if "is_in_print" in model_args:
            model_args["is_in_print"] = (
                True if model_args["is_in_print"] == "True" else False
            )
